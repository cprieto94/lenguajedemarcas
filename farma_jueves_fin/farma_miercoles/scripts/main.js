...
function peticionAsincrona(metodoCallback){
    ...
    http.onreadystatechange = function(){
        ...
        if(this.readyState == 4 && this.status == 200){
            ...
            console.error("La petición está fallando");
        }
    }
    http.send()
}
...
function metodoPintarFarmaciasGrid(response){
    ...
    lista.forEach(far => {
        ...
        htmlGenerado += '<div><p onclick="loadDetails('+far.id+')" class="mas-info" id="'+ far.id+'">Más información</p></div>'
        htmlGenerado +='</div>';
        contenedor.innerHTML = htmlGenerado;
    });
...
function manejadorLoadDetails(){
    ...
    //Necesito saber el identificador de la farmacia
    const id = localStorage.getItem("idFarmacia");

    const url = 'https://www.zaragoza.es/sede/servicio/farmacia/'+id+'?rf=html&srsname=wgs84';
